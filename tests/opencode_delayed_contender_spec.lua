...
if mode == "worker" then
  ...
  if vim.env.MKCHAD_OPENCODE_EXPECT_FALLBACK == "1" then
    assert(
      state.port ~= 4096 and state.port_source == "fallback",
      "contenders must converge on a persisted high public fallback"
    )
  end
...
if mode == "cleanup" then
  ...
  if state and state.schema == 4 then
    local done, stopped, stop_err = false, nil, nil
    lifecycle.stop_server(function(ok, err)
      stopped, stop_err, done = ok, err, true
    end)
    assert(
      vim.wait(10000, function()
        return done
      end, 10),
      "broker cleanup timed out"
    )
    assert(stopped, stop_err)
    state, pending = lifecycle.read_state(), lifecycle.read_pending()
  end
